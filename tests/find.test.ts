...
describe("discovery_find", () => {
  it("builds a generic search with a relationship scope", () => {
    const query = buildFindQuery({ kind: "SoftwareInstance", relatedToKind: "Host", relatedToName: "db01" } as never);
  ...
  it("matches a free term across all attributes via the wildcard", () => {
    const query = buildFindQuery({ kind: "Host", contains: "Windows 2019" } as never);
  ...
  it("derives the host role and returns visual content when listing a host's software", async () => {
    ...
    const result = await findTools(client).discovery_find.handler({ kind: "SoftwareInstance", relatedToKind: "Host", relatedToName: "db01" });
  ...
  it("rejects an unknown kind with suggestions and does not query", async () => {
    ...
    const result = await findTools(client).discovery_find.handler({ kind: "Hostt" });
