...
export function registerDiscoveryPrompts(server: McpServer): void {
  ...
  server.registerPrompt(
    ...
    ({ workload, current_solution }) => userMessage([
      ...
      "",
      "Pour un périmètre réel (un service, un hôte, ou tout le parc virtuel), préfère discovery_cost(mode=compare, scope={type:'service'|'host'|'fleet', name:'...'}) : il résout les VMs depuis Discovery, les bucketise par taille (vCPU/RAM) et compare on-prem vs cloud sans saisir la quantité.",
