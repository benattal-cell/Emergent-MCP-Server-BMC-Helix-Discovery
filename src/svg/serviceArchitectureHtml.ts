...
#missing{position:fixed;z-index:5;left:50%;top:50%;transform:translate(-50%,-50%);background:var(--panel);border:1px solid var(--border);padding:14px;border-radius:6px;display:none;max-width:520px}
</style></head><body>
<div id="topbar"><div class="title">${safeTitle}</div><button id="fit">Fit</button><button id="theme">🌙</button></div>
<svg id="chart"></svg><div id="tip"><div class="tip-kind"></div><div class="tip-name"></div><div class="tip-extra"></div></div><div id="missing">D3 bundle unavailable. Install the runtime dependency and rebuild/start the server so this offline architecture diagram can render.</div>
