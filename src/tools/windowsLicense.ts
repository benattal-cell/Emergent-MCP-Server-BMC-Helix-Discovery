...
function hostShowColumns(role: PhysicalHostInventoryRow["role"]): string {
  ...
  return [
    ...
    "#Host:Hardware:ReferenceData:ReferenceData:HardwareReferenceData.vendor AS 'hrd_vendor'",
    "#Host:Hardware:ReferenceData:ReferenceData:HardwareReferenceData.model AS 'hrd_model'",
    "#Host:Hardware:ReferenceData:ReferenceData:HardwareReferenceData.config_num_processors AS 'hrd_config_num_processors'",
...
export function dedupeHosts(hosts: PhysicalHostInventoryRow[]): PhysicalHostInventoryRow[] {
  ...
  for (const host of hosts) {
    ...
    else byId.set(host.id, { ...host, ...existing, windowsVmCount: Math.max(host.windowsVmCount ?? 0, existing.windowsVmCount ?? 0), totalVmCount: Math.max(host.totalVmCount ?? 0, existing.totalVmCount ?? 0) });
  }
  return [...byId.values()];
}
...
export function findOptimizationOpportunities(hosts: LicensedHost[], input: LicenseParameters): OptimizationOpportunity[] {
  ...
  for (const [clusterId, clusterHosts] of clusters) {
    ...
    const targetHosts = Math.max(1, Math.ceil(totalWindowsVms / Math.max(1, input.underloadedVmThreshold * 2)));
    ...
    if (currentCost > optimizedCost) {
      opportunities.push({
        ...
        evidence: `${totalWindowsVms} VM Windows réparties sur ${clusterHosts.length} ESX; consolidation théorique sur ${targetHosts} hôte(s) Windows.`
      });
      opportunities.push({
        ...
        optimizedCost,
        saving: currentCost - optimizedCost,
        evidence: `Créer un pod d'affinité Windows de ${targetHosts} ESX au lieu de licencier tout le cluster.`
      });
    }
  }
