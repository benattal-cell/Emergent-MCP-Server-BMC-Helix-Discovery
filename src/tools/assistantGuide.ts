    ...
    items: [
      {
        ...
        purposeEn: "IT costs / Value Review, in 4 modes (`mode` param). categories: catalog families. search: components by text/category/subcategory. estimate: min/median/max for a `component` × `quantity`. compare: annualized alternatives + savings versus `current_solution`. In estimate/compare, `scope` (service/host/fleet) prices a REAL fleet: resolves the VMs from Discovery, buckets them by size (vCPU/RAM), derives the quantity.",
